...
// Dati di test con la struttura JSON richiesta
const TEST_POSTS = [
  {
...
// Funzione per testare il webhook Instagram
async function testInstagramWebhook(posts) {
...
async function runTest() {
  try {
    console.log('🚀 Iniziando test webhook Instagram...\n');
    ...
    const result = await testInstagramWebhook(TEST_POSTS);
    
    console.log('\n🎉 Test completato con successo!');
    console.log(`⏱️  Tempo totale: ${result.duration}ms`);
    console.log(`📊 Video processati: ${result.result.processed || 0}`);
    ...
    if (result.result.results) {
      ...
      result.result.results.forEach((item, index) => {
        if (item.success) {
          console.log(`  ✅ [${index + 1}] ${item.post_id}: ${item.audio_size_mb}MB`);
        } else {
...
module.exports = { testInstagramWebhook }; 